...
class follower:
    def __init__(self, followDistance=1.2, stopDistance=0.7, max_speed=0.4, min_speed=0.01 ):
	...
	# When following, it publishes to Bark.py if the human comes closer to the robot
	    if (self.closest < 0.9*self.stopDistance and self.closest > 0.8*self.stopDistance):
	...
	# Two distance ranges are defined and the sound reproduced will be different for each range 
	    elif (self.closest < 0.8*self.stopDistance and self.closest > 0.7*self.stopDistance):
	    	self.pubbark = rospy.Publisher('follow', String)
